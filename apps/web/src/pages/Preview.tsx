...
function AiPromptLogModal({
  ...
  const [items, setItems] = useState<AiPromptAudit[]>([]);
  ...
  useEffect(() => {
    ...
    api
      .listAiPrompts(runId)
      .then((edits) => {
        if (!cancelled) setItems(edits);
      })
      .catch((err) => {
        if (cancelled) return;
        setError(err instanceof ApiError ? err.message : "Couldn't load prompts.");
      })
      .finally(() => {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      widthClass="w-[760px]"
      labelledBy="ai-prompt-log-title"
    >
      <ModalHeader
      ...
      <div className="p-5 space-y-4">
        {loading && <div className="text-sm text-ink-muted">Loading prompts…</div>}
        {error && <div className="text-sm text-error">{error}</div>}
        {!loading && !error && items.length === 0 && (
          <div className="text-sm text-ink-muted">
            No AI prompts have been recorded for this newsletter.
          </div>
        )}
        {!loading &&
          !error &&
          items.map((item) => (
            <article
              key={item.id}
              className="border border-rule rounded-md overflow-hidden bg-bg"
            >
              <div className="px-3 py-2 border-b border-rule flex flex-wrap items-center gap-2 text-xs">
                <Tag tone={promptStatusTone(item.resultStatus)}>
                  {promptStatusLabel(item.resultStatus)}
                </Tag>
                {item.diffSummary?.provider && (
                  <Tag tone="blue">{formatProvider(item.diffSummary)}</Tag>
                )}
                {item.diffSummary?.durationMs !== undefined && (
                  <Tag tone="neutral">{formatDuration(item.diffSummary.durationMs)}</Tag>
                )}
                <span className="text-ink-muted ml-auto">
                  {new Date(item.createdAt).toLocaleString()}
                </span>
                {promptMetaSummary(item) && (
                  <div className="basis-full text-2xs text-ink-muted">
                    {promptMetaSummary(item)}
                  </div>
                )}
              </div>
              <pre className="max-h-[320px] overflow-auto whitespace-pre-wrap p-3 text-xs leading-5 font-mono text-ink">
                {item.prompt}
              </pre>
            </article>
          ))}
...
function promptStatusLabel(status: string): string {
  return {
    "generation-content": "Mock content",
    "generation-ai": "Generated by AI",
    "generation-fallback": "V3 fallback",
    "generation-filler-ai": "AI filler",
    "generation-filler-fallback": "Filler fallback",
    ok: "AI edit",
    fallback: "Edit fallback",
  }[status] ?? status;
}

function promptStatusTone(status: string): "blue" | "green" | "warn" {
  if (status.includes("fallback")) return "warn";
  if (status === "generation-content") return "blue";
  return "green";
}

function formatProvider(meta: NonNullable<AiPromptAudit["diffSummary"]>): string {
  const provider = {
    gemini: "Gemini",
    openai: "OpenAI",
    newsforge: "NewsForge",
  ...
  return [provider, meta.model].filter(Boolean).join(" · ");
}
...
function promptMetaSummary(item: AiPromptAudit): string | null {
  ...
  if (!meta) return null;
  if (meta.kind === "generation-content") {
    return `Created ${meta.articles ?? "the"} articles and ${meta.images ?? "starter"} images for assembly.`;
  }
  if (meta.kind === "generation-design") {
    return [
      meta.mode ? `Design mode: ${meta.mode}` : null,
      meta.templateId ? `Template: ${meta.templateId}` : null,
      meta.designNotes,
      meta.fallbackReason ? `Fallback: ${meta.fallbackReason}` : null,
    ]
      .filter(Boolean)
      .join(" · ");
  }
  if (meta.kind === "edit" && "summary" in meta && typeof meta.summary === "string") {
    return meta.summary;
  }
  return meta.fallbackReason ? `Fallback: ${meta.fallbackReason}` : null;
}
