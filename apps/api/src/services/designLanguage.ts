...
7. Birthdays are client-supplied only. If an uploaded article explicitly contains a birthday roster, convert it to kind "list" with listItems: group headers ("RESIDENTS", "STAFF") as isGroupHeader rows, then {label: "First L.", value: "M/D"} rows. Style: bg "sun", panelRole "birthday", scriptHeading, heading "Happy Birthday!". If no explicit roster exists, leave a "Birthday List Placeholder" client-fill area and never invent birthday names, dates, or celebration copy.
